...
class AdminCodeDataSheet(DataExcelSheet):
    ...
    def classification(self):
        ...
        self._prefectures = pd.DataFrame(
            {'acode': list(self.prefectures.index), 'region': list(self.prefectures['region'])})
        self._counties = pd.DataFrame({'acode': list(self.counties.index), 'region': list(self.counties['region'])})

        # 创建字典形式的三级行政区域，用来插入MongoDB数据库
        self.provinces_dict = [
            {'acode': str(int(self._provinces.loc[i][0])), 'region': self._provinces.loc[i][1], 'adminlevel': 2,
             'version': self.version, 'year': self.year} for i in self._provinces.index]
        self.prefectures_dict = [
            {'acode': str(int(self._prefectures.loc[i][0])), 'region': self._prefectures.loc[i][1], 'adminlevel': 3,
             'version': self.version, 'year': self.year} for i in self._prefectures.index]
        self.counties_dict = [
            {'acode': str(int(self._counties.loc[i][0])), 'region': self._counties.loc[i][1], 'adminlevel': 4,
             'version': self.version, 'year': self.year} for i in self._counties.index]
