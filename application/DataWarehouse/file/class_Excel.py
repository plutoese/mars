...
class Excel:
    ...
    @property
    def sheetname(self):
        with xlrd.open_workbook(self.filename) as mfile:
    ...
    def read(self, sheet=None):
        ...
        with xlrd.open_workbook(self.filename) as self.file:
            ...
            # 读取数据
            self.data = [single_sheet.row_values(
                i) for single_sheet in self.sheet for i in range(single_sheet.nrows)]
...
if __name__ == '__main__':
    filename = 'E:/data/testdata/excel_data.xls'
    mexcel = Excel(filename)
    mdata = mexcel.read()
    print(len(mdata))
    print(mexcel.sheet)
    ...
    print(os.getcwd())
    outfile = os.getcwd() + '\\static\\file\\' + 'demo.xlsx'
    moutexcel = Excel(outfile)
